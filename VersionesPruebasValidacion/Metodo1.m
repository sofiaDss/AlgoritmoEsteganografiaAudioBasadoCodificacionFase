%{
    Método #1. Modificación de todas las muestras del espectro de fase
    Contiene Tx y Rx.
   ...
   archivosP = dir(strcat('Portada/',carpetasP(i).name,'/*.mp3'));
    ...
    for j = 1:length(archivosP)     %iteración por los arhivos de audio de la carpeta
        ...
        for k=1:length(archivosS) 
            ...
            ruta_stego=sprintf('Stego/metodo1/%s/%s',carpetasP(i).name,nombre_stego);
            ...
            ruta_resultado=sprintf('ResultadoSecreto/metodo1/%s',nombre_recuperado);
...
writetable(T,'informe.xlsx','sheet','Metodo1','Range','A2');

% as=audioplayer(xs,Fs); 

% tiempoPortada=50;
% tiempoSecreto=(tiempoPortada*48000)/(41100*2)
